...
import React, { forwardRef, useCallback, useImperativeHandle, useMemo, useRef, useState, useLayoutEffect, useEffect } from 'react';
import { Animated, View, PanResponder, StyleSheet, useWindowDimensions, Keyboard, Platform } from 'react-native';
import { DEFAULT_ANIMATION, DEFAULT_BACKDROP_MASK_COLOR, DEFAULT_CLOSE_ANIMATION_DURATION, DEFAULT_HEIGHT, DEFAULT_OPEN_ANIMATION_DURATION } from '../../constant';
 ...
 */
const BottomSheet = /*#__PURE__*/forwardRef((_ref, ref) => {
  let {
  ...
  const Animators = useMemo(() => ({
    ...
    animateBackdropMaskOpacity(toValue, duration) {
      ...
      const _duration = animationType === ANIMATIONS.FADE ? duration : duration / 2.5;
    ...
    animateHeight(toValue, duration) {
      return Animated.timing(_animatedHeight, {
        ...
        useNativeDriver: false,
        duration: duration,
        easing: customEasingFunction && typeof customEasingFunction === 'function' ? customEasingFunction : animationType === ANIMATIONS.SLIDE ? this._slideEasingFn : this._springEasingFn
      });
    }
  }), [animationType, customEasingFunction, _animatedContainerHeight, _animatedBackdropMaskOpacity, _animatedHeight]);
  const interpolatedOpacity = useMemo(() => animationType === ANIMATIONS.FADE ? _animatedBackdropMaskOpacity.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [0, 0.3, 1],
    extrapolate: 'clamp'
  ...
  const convertedHeight = useMemo(() => {
    ...
    if (sheetOpen && newHeight !== curHeight) {
      if (animationType === ANIMATIONS.FADE) _animatedHeight.setValue(newHeight);else Animators.animateHeight(newHeight, newHeight > curHeight ? openDuration : closeDuration).start();
    }
    return newHeight;
   ...
   */
  const keyboardHandler = useHandleKeyboardEvents(!disableKeyboardHandling, convertedHeight, sheetOpen, Animators.animateHeight, contentWrapperRef);
    ...
    return PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        if (gestureState.dy > 0) {
          ...
          if (animationType !== ANIMATIONS.FADE) _animatedHeight.setValue(convertedHeight - gestureState.dy);
        }
      },
      onPanResponderRelease(_, gestureState) {
        if (gestureState.dy >= convertedHeight / 3 && closeOnDragDown) {
          ...
          if (animationType !== ANIMATIONS.FADE) Animators.animateHeight(convertedHeight, openDuration / 2).start();
        }
      }
    }).panHandlers;
  };
  ...
  const PolymorphicHandleBar = () => {
    ...
    return hideDragHandle ? null : CustomHandleBar && typeof CustomHandleBar === 'function' ? /*#__PURE__*/React.createElement(View, _extends({
      style: {
        alignSelf: 'center'
      }
    }, panHandlersFor('handlebar')), /*#__PURE__*/React.createElement(CustomHandleBar, {
      _animatedHeight: _animatedHeight
    })) : /*#__PURE__*/React.createElement(DefaultHandleBar, _extends({
   ...
   */
  useHandleAndroidBackButtonClose(android_closeOnBackPress, closeBottomSheet, sheetOpen);

  // Children
  const ChildNodes = typeof Children === 'function' ? /*#__PURE__*/React.createElement(Children, {
    _animatedHeight: _animatedHeight
  }) : Children;
  return /*#__PURE__*/React.createElement(React.Fragment, null, typeof passedContainerHeight === 'string' ?
  /*#__PURE__*/
  /**
  ...
  }, modal ? /*#__PURE__*/React.createElement(Backdrop, {
  ...
  }, panHandlersFor('contentwrapper')), /*#__PURE__*/React.createElement(PolymorphicHandleBar, null), /*#__PURE__*/React.createElement(View
...
const materialStyles = StyleSheet.create({
  ...
  contentContainerShadow: Platform.OS === 'android' ? {
    elevation: 7
  } : {
    shadowColor: '#000',
    shadowOffset: {
